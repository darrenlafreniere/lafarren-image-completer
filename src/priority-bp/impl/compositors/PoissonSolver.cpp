...
namespace PriorityBp { namespace Poisson
{
	...
	{
		return Clamp0To1(static_cast<float>(m_uVars.get()[index]));
	}
		...
		int sourceOffsetY)
	{
		...
		for (int y = 0; y < sourceImageTranslatedHeight; ++y)
		{
			for (int x = 0; x < sourceImageTranslatedWidth; ++x)
			{
				...
				{
					const int sourceIndex = GetRowMajorIndex(sourceWidth, translatedX, translatedY);
					const int translatedIndex = GetRowMajorIndex(sourceImageTranslatedWidth, x, y);
