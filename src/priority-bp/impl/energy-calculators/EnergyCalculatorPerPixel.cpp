...
static inline Energy CalculateEnergy(
	const Image& inputImage, const MaskLod* mask,
	...
	int bLeft, int bTop)
{
	...
	if (width > 0 && height > 0)
	{
		...
		int aRowIndex = GetRowMajorIndex(imageWidth, aLeft, aTop);
		int bRowIndex = GetRowMajorIndex(imageWidth, bLeft, bTop);
...
//
EnergyCalculatorPerPixel::EnergyCalculatorPerPixel(const Image& inputImage, const MaskLod& mask) :
...
EnergyCalculator::BatchImmediate EnergyCalculatorPerPixel::BatchOpenImmediate(const BatchParams& params)
{
...
EnergyCalculator::BatchQueued EnergyCalculatorPerPixel::BatchOpenQueued(const BatchParams& params)
{
	wxASSERT(m_batchState == BatchStateClosed);
...
void EnergyCalculatorPerPixel::BatchClose()
{
	wxASSERT(m_batchState != BatchStateClosed);
	m_batchState = BatchStateClosed;
	m_isAsyncBatch = false;
...
Energy EnergyCalculatorPerPixel::Calculate(int bLeft, int bTop) const
{
	wxASSERT(m_batchState != BatchStateClosed);
...
EnergyCalculator::BatchQueued::Handle EnergyCalculatorPerPixel::QueueCalculation(int bLeft, int bTop)
{
	wxASSERT(m_batchState != BatchStateClosed);
...
void EnergyCalculatorPerPixel::ProcessCalculations()
{
...
Energy EnergyCalculatorPerPixel::GetResult(BatchQueued::Handle handle) const
{
	wxASSERT(m_batchState == BatchStateOpenQueuedAndProcessed);
	return m_queuedCalculationsAndResults[handle].result;
}

Energy EnergyCalculatorPerPixel::CalculateNoMask(int bLeft, int bTop) const
{
...
Energy EnergyCalculatorPerPixel::CalculateMaskA(int bLeft, int bTop) const
{
...
EnergyCalculatorPerPixel::QueuedCalculationAndResultIndexBuffer::QueuedCalculationAndResultIndexBuffer(EnergyCalculatorPerPixel& energyCalculatorPerPixel) :
m_energyCalculatorPerPixel(energyCalculatorPerPixel)
{
}

void EnergyCalculatorPerPixel::QueuedCalculationAndResultIndexBuffer::ProcessCalculationsAndClear()
{
...
EnergyCalculatorPerPixel::WorkerThread::WorkerThread(EnergyCalculatorPerPixel& energyCalculatorPerPixel) :
...
wxThread::ExitCode EnergyCalculatorPerPixel::WorkerThread::Entry()
{
	State state = Active;
	while (state != Quitting)
	{
		...
		if (state == Pausing)
		{
			...
			{
				const State previousState = State(Atomic<>::CompareExchange(&m_state, Paused, Pausing));
			...
			if (state == Resuming)
			{
				const State previousState = State(Atomic<>::CompareExchange(&m_state, Active, Resuming));
...
bool EnergyCalculatorPerPixel::WorkerThread::IsPaused() const
{
	return wxThread::IsPaused();
}

void EnergyCalculatorPerPixel::WorkerThread::ResumeAndStartProcessingCalculations()
{
	wxASSERT(IsPaused());

	const State previousState = State(Atomic<>::CompareExchange(&m_state, Resuming, Paused));
...
void EnergyCalculatorPerPixel::WorkerThread::FinishProcessingCalculationsAndPause()
{
	wxASSERT(!IsPaused());
	...
	while (Atomic<>::CompareExchange(&m_state, Pausing, Active) != Active)
	{
		wxThread::Yield();
	}
...
void EnergyCalculatorPerPixel::WorkerThread::ResumeAndQuit()
{
	wxASSERT(IsPaused());

	const State previousState = State(Atomic<>::CompareExchange(&m_state, Quitting, Paused));
	wxASSERT(previousState == Paused);
...
EnergyCalculatorPerPixel::WorkerThread::State EnergyCalculatorPerPixel::WorkerThread::AtomicGetState() const
{
	// The exchange and comparand are dummies.
	return State(Atomic<>::CompareExchange(&m_state, Invalid, Invalid));
}
